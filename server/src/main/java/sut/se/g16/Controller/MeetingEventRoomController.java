...
public class MeetingEventRoomController {
    ...
    public Boolean addMeetingEventRoom(@PathVariable String name, @PathVariable String meetingRoomType,
            ...
            @PathVariable int number, @PathVariable int price, @PathVariable String memberUserName) {
        if (count != 0) {
    ...
    @PathVariable Long id1, @PathVariable Long id2, @PathVariable Long id3) {
        ...
        int meetingEventRoomNumber = meetingEventRoomRepository.findRoomNumberById(id);
        ...
        if (meetingEventRoomNumber == number) {
            checkDoubly = 0;
        } else {
